...
export function normalizeBoard(data: Partial<Board> & { id: number; name: string }): Board {
  return {
    ...
    chainLocked: data.chainLocked ?? data.permissions?.chainLocked,
    chainMessage: data.chainMessage ?? data.permissions?.chainMessage,
    parentBoardName: data.parentBoardName,
    canPlanMilestone: data.canPlanMilestone,
    canPlanSprint: data.canPlanSprint,
    linkedDefectBoardId: data.linkedDefectBoardId,
    linkedRetroBoardId: data.linkedRetroBoardId,
    linkedSprintId: data.linkedSprintId,
    linkedSprintName: data.linkedSprintName,
